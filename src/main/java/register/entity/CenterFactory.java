...
public class CenterFactory {
     ...
     * 在本机注册的所有索引服务器
     */
    private static final Set<ServerIndexNode> serverNodes=new HashSet<>();

    /**
     * 所有客户端
     */
    private static final Set<ClientNode> client = new HashSet<>();

    /**
     * 所有服务器相同的注册结点集合
     */
    protected static final Set<RegNode> regNodes=new HashSet<>();
    ...
    public static void regClient(ClientNode clientNode){
        client.add(clientNode);
    }

    public static ClientNode[] listClient(){
        return client.toArray(new ClientNode[0]);
    }
    ...
    public static void regClients(ClientNode[] clientNodes){
        client.addAll(Arrays.asList(clientNodes));
    }
    ...
    public static void regRegNodes(RegNode[] nodes){
        regNodes.addAll(Arrays.asList(nodes));
    }
    ...
    public synchronized  static void regNodes(IndexNode indexNode){
        ...
        serverNodes.add(new ServerIndexNode(indexNode));
    }
    ...
    public synchronized static void liveNodeHeartBeat(IndexNode indexNode){
        for (ServerIndexNode node1:serverNodes){
    ...
    public synchronized static RegNode[] listRegNode(){
        return regNodes.toArray(new RegNode[0]);
    }
